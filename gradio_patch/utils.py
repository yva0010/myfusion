...
def get_type(schema):
    if not isinstance(schema, dict):
        # Handle cases like `additionalProperties: true` or false
        return str(schema)  # or return "any" if you prefer
    if "const" in schema:
        return "const"
    if "enum" in schema:
        return "enum"
...
def _json_schema_to_python_type(schema: Any, defs) -> str:
    ...
    if isinstance(schema, bool):
      # Handle True/False schemas — allowed by JSON Schema spec
      return "any" if schema else "never"

    if not isinstance(schema, dict):
        raise APIInfoParseError(f"Invalid schema type: {type(schema)}: {schema}")
