...
from myfusion.common_helper import calc_float_step, calc_int_step
from myfusion.face_analyser import get_many_faces
from myfusion.face_selector import sort_and_filter_faces
from myfusion.face_store import clear_reference_faces, clear_static_faces
from myfusion.filesystem import is_image, is_video
from myfusion.typing import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
from myfusion.uis.core import get_ui_component, get_ui_components, register_ui_component
from myfusion.uis.typing import ComponentOptions
from myfusion.uis.ui_helper import convert_str_none
from myfusion.vision import get_video_frame, normalize_frame_color, read_static_image
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = myfusion.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = myfusion.choices.face_selector_orders,
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + myfusion.choices.face_selector_genders,
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = ['none'] + myfusion.choices.face_selector_races,
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or myfusion.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or myfusion.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = wording.get('uis.face_selector_age_range_slider'),
				minimum = myfusion.choices.face_selector_age_range[0],
				maximum = myfusion.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calc_int_step(myfusion.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calc_float_step(myfusion.choices.reference_face_distance_range),
		minimum = myfusion.choices.reference_face_distance_range[0],
		maximum = myfusion.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
