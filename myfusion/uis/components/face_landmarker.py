...
from myfusion import face_landmarker, state_manager, wording
from myfusion.common_helper import calc_float_step
from myfusion.typing import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = myfusion.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(myfusion.choices.face_landmarker_score_range),
		minimum = myfusion.choices.face_landmarker_score_range[0],
		maximum = myfusion.choices.face_landmarker_score_range[-1]
	)
