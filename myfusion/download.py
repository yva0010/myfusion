...
from myfusion.filesystem import get_file_size, is_file, remove_file
from myfusion.hash_helper import validate_hash
from myfusion.typing import DownloadProvider, DownloadSet
...
def conditional_download_sources(sources: DownloadSet) -> bool:
	...
	for invalid_source_path in invalid_source_paths:
		...
		if invalid_source_file_name == "open_nsfw":
			logger.warning(f"[PATCHED] Validation failed for {invalid_source_file_name}, but skipping deletion.")
			continue
...
def ping_download_provider(download_provider : DownloadProvider) -> bool:
	download_provider_value = myfusion.choices.download_provider_set.get(download_provider)
